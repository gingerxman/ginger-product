...
func (this *UpdateDisplayIndexService) doUp(item *ItemPos, extraFilters eel.Map) error {
	...
	sql := fmt.Sprintf("select id, display_index, original_display_index from %s where %s and display_index < ? order by display_index desc limit 1", table, getExtraFilterCondition(extraFilters))
	db := o.Raw(sql, item.DisplayIndex).Scan(&targetItem)
	err := db.Error
	if err != nil {
	...
	err = o.Exec(sql, item.DisplayIndex, targetItem.Id).Error
	if err != nil {
		eel.Logger.Error(err)
		return err
	}
	//调整item的display_index
	err = o.Exec(sql, targetItem.DisplayIndex, item.Id).Error
	if err != nil {
		eel.Logger.Error(err)
		return err
	}
...
func (this *UpdateDisplayIndexService) doDown(item *itemPos, extraFilters eel.Map) error {
	...
	//获得与item进行位置交换的targetItem
	sql := fmt.Sprintf("select id, display_index, original_display_index from %s where %s and display_index > ? order by display_index limit 1", table, getExtraFilterCondition(extraFilters))
	err := o.Raw(sql, item.DisplayIndex).Scan(&targetItem).Error
	if err != nil {
		eel.Logger.Error(err)
		return err
	}
...
func (this *UpdateDisplayIndexService) setTop(item *itemPos, extraFilters eel.Map) error {
	...
	//获得排在第一位的targetItem
	sql := fmt.Sprintf("select id, display_index, original_display_index from %s where %s and display_index > ? order by display_index limit 1", table, getExtraFilterCondition(extraFilters))
	err := o.Raw(sql, -STICK_BOUNDARY).Scan(&targetItem).Error
	if err != nil {
		eel.Logger.Error(err)
		return err
	}
	...
	sql = fmt.Sprintf("update %s set display_index = ? where id = ?", table)
	err = o.Exec(sql, targetItem.DisplayIndex-1, item.Id).Error
	if err != nil {
		eel.Logger.Error(err)
		return err
	}
...
func (this *UpdateDisplayIndexService) stickTop(item *itemPos, extraFilters eel.Map) error {
	...
	err := o.Exec(sql, item.DisplayIndex-2*STICK_BOUNDARY, item.DisplayIndex, item.Id).Error
	if err != nil {
		eel.Logger.Error(err)
		return err
	}
...
func (this *UpdateDisplayIndexService) unstick(item *itemPos, extraFilters eel.Map) error {
	...
	err := o.Raw(sql, item.OriginalDisplayIndex, item.Id).Error
	if err != nil {
		eel.Logger.Error(err)
		return err
	}
...
//Encode 对单个实体对象进行编码
func (this *UpdateDisplayIndexService) Update(item *itemPos, action string, extraFilters eel.Map) error {
	return this.UpdateWithExtraFilter(item, action, extraFilters)
}
