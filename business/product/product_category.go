...
func (this *ProductCategory) UpdateDisplayIndex(action string) error {
	...
	filters := eel.Map{
		...
		"node_type": m_product.STR2PRODUCTCATEGORYNODE[this.NodeType],
	}
	err := NewUpdateDisplayIndexService(this.Ctx, DISPLAY_INDEX_ORDER_ASC).Update(&item, action, filters)
	if err != nil {
		eel.Logger.Error(err)
		return err
	}
