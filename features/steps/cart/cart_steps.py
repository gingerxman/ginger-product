...
def get_shopping_cart_item_id_by_product_name(product_name):
	...
	shopping_cart_item = bdd_util.exec_sql("select * from cart_item where pool_product_id = %s", [pool_product_id])[0]
...
def step_impl(context, user, corp_name):
	...
	for product_data in input_datas:
		...
		response = context.client.put('cart.cart_item', data)
		bdd_util.assert_api_call_success(response)
		time.sleep(0.1)

@then(u"{user}能获得购物车")
def step_impl(context, user):
	response = context.client.get("cart.cart", {})
	bdd_util.assert_api_call_success(response)
...
def step_impl(context, user):
	...
	for product_name in input_datas:
		...
		response = context.client.delete('cart.cart_item', data)
		bdd_util.assert_api_call_success(response)

@then(u"{user}能获得购物车中商品数量为'{count}'")
def step_impl(context, user, count):
	response = context.client.get("cart.product_count", {})
	bdd_util.assert_api_call_success(response)
