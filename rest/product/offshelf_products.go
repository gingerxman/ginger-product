...
func (this *OffshelfProducts) GetParameters() map[string][]string {
	return map[string][]string{
		"GET": []string{},
		"PUT": []string{"pool_product_ids:json-array"},
	}
}
...
func (this *OffshelfProducts) Put(ctx *eel.Context) {
	...
	bCtx := ctx.GetBusinessContext()
	corp := account.GetCorpFromContext(bCtx)
	poolProductIds := req.GetIntArray("pool_product_ids")
	
	poolProducts := product.GetProductPoolForCorp(bCtx, corp).GetPoolProductsByIds(poolProductIds)
